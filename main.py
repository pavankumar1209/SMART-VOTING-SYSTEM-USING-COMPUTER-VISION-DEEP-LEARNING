...
@ app.route('/voterlog',methods=['POST','GET'])
def voterlog():
    if request.method=='POST':
        name=request.form['name']
        aadhaar=request.form['aadhaar']
        sql="select count(*) from voters where first_name='"+name+"' and aadhar_id='"+aadhaar+"'"
        ...
        if count !=0:
            ...
            return render_template('voterlog.html')
    return render_template('voterlog.html')
...
def getImagesAndLabels(path):
    ...
    output = open('models/ncoder.pkl', 'wb')
...
def train():
    if request.method=='POST':
        ...
        recognizer.save("models/Trained.yml")
...
def voting():
    if request.method=='POST':
        pkl_file = open('models/ncoder.pkl', 'rb')
        my_le = pickle.load(pkl_file)
        pkl_file.close()
        recognizer = cv2.face.LBPHFaceRecognizer_create()  # cv2.createLBPHFaceRecognizer()
        recognizer.read(r"models/Trained.yml")
        cam=cv2.VideoCapture(0, cv2.CAP_DSHOW)
        ...
        while True:
            ...
            if len(faces) > 1:
                print(f"More than one face detected: {len(faces)} faces. Stopping camera...")
                cv2.waitKey(2000)  # Wait 2 seconds to allow users to see the message
                cv2.destroyAllWindows()
                flash(f'More than one face detected: {len(faces)} faces. Stopping camera...')
                return render_template('voting.html')
...
def select_candidate():
    ...
    sq = pd.read_sql_query("select AES_DECRYPT(aadhar,'voting') as aadhar from vote",mydb)
    all_adhar = pd.DataFrame(sq['aadhar'].values)
    does_exist = False
    for i in sq['aadhar'].values:
        i = i.decode()
        if i == aadhar:
            does_exist = True
            break
    ...
    if does_exist:
        ...
        if request.method == 'POST':
            ...
            session['vote'] = vote
            result = hashlib.sha1(vote.encode())
            hash1 = result.hexdigest()
            sql = "INSERT INTO vote (vote, aadhar,vote_data) VALUES (AES_ENCRYPT('%s','voting'), AES_ENCRYPT('%s','voting'),'%s')" % (vote, aadhar,hash1)
...
@app.route('/view_results_verify', methods=['POST','GET'])
def view_results_verify():
    if request.method=='POST':
        otp_check=request.form['otp_check']
        if otp_check==session['otp']:

            flash(r"OTP verified successfully",'primary')
            return redirect(url_for('voting_res')) #change it to capture photos
        else:
            flash(r"Wrong OTP. Please try again.","info")
            return redirect(url_for('view_results_verify'))
    else:
            #Sending OTP
        message = MIMEMultipart()
        receiver_address = "arravalipavan@gmail.com"
        message['From'] = sender_address
        message['To'] = receiver_address
        Otp = str(np.random.randint(100000, 999999))
        session['otp']=Otp
        ...
        abc.sendmail(sender_address, receiver_address, text)
        abc.quit()
    return render_template('view_results_verify.html')



@app.route('/voting_res')
def voting_res():
    votes = pd.read_sql_query("select AES_DECRYPT(vote, 'voting') as vote, AES_DECRYPT(aadhar, 'voting') as aadhar from vote", mydb)
    print(votes.iloc[0:1,:1])
    counts = pd.DataFrame(votes['vote'].value_counts())
    counts.reset_index(inplace=True)
    all_imgs=[b'1.png',b'2.png',b'3.jpg',b'4.png',b'5.png',b'6.png']
